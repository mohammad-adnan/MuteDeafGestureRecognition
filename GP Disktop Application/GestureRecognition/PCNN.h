...
public:
	...
	int iterate(RImage A) {
		...
		F->mul((float) exp(-1.0 / af));
		F->add(mul1(vf, work));
		F->add(A);
		L->mul((float) exp(-1.0 / al));
		L->add(mul1(vl, work));
		U = new RImage(mul1(beta, L)->add((float) 1.0)->mul(F)->getData(),vert,horz);
		for (int i = 0; i < vert; i++)
			for (int j = 0; j < horz; j++)
				Y->set(i, j, (float) (((U->get(i, j) > T->get(i, j)) ? 1.0 : 0.0)));

		T->mul((float) exp(-1.0 / at));
		T->add(mul1(vt, Y));
